...
public class OrderTaxTest {
  ...
  public void testTaxOnlineOrder() {
    Order order = new Order("online1", Location.ONLINE);
  ...
  public void testTaxEuropeOrder() {
    Order order = new Order("europe1", Location.EUROPE);
    order.processCart(cart1);
  ...
  public void testTaxUSAOrder() {
    Order order = new Order("usa1", Location.USA);
    order.processCart(cart1);
    assertEquals(0, order.getTax(), 0.001);
